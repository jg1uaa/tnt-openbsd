/* tnt: Hostmode Terminal for TNC
   ...
   updated: Matthias Hensler, 99/09/08
*/
...
extern void cmd_actiface_ext();
extern void cmd_display();
extern void connstr_to_socket();
extern void get_iface_name();
extern void no_success_connect();
extern int queue_cmd_data();
extern void rem_data_display();
extern void send_qrginfo_all();
extern void set_pwmode();
extern void set_remmode();
extern void socksetup_timeout();
extern void socksetup_to_conn();
extern void stat_display();
extern void statlin_update();
extern void strip_call();
extern void update_name();
extern void update_remcall();
extern void update_ssid();
...
char *str;
{
  ...
  if (ax25k_active) {
    cmd_display(mode,channel,
                _("Error: To change QRGs, do an update of 'axports'"),1);
    return;
  }
  ...
  if ((res >= 2) && (res <= 3) && (port < MAXQRGS)) {
    ...
    while (i < MAXQRGS) {
      if ((port != i) && (qrg_info[i].qrg[0] == '\0')) {
        cmd_display(mode,channel, _("invalid portnumber"),1);
        return;
      }
      if (port == i) {
        if (strcmp(qrg,"$") == 0) {
          if (qrg_info[i+1].qrg[0] != '\0') {
            cmd_display(mode,channel, _("not allowed to delete"),1);
            return;
          }
          qrg_info[i].qrg[0] = '\0';
          qrg_info[i].port_str[0] = '\0';
  ...
  cmd_display(mode,channel, _("Invalid syntax"),1);
}
...
char *port;
{
  ...
  offset = 0;
  while ((*(qrg+offset) == ' ') || (*(qrg+offset) == 0x09)) {
    offset++;
  }
  /* copy data */
  strncpy(qrg_info[i].qrg,qrg+offset,19);
  qrg_info[i].qrg[19] = '\0';

/* tnt: Hostmode Terminal for TNC
   ...
   * p2c, the Pascal-to-C translator *
   ...
   $Id: bcastadd.c,v 1.2 2001/11/28 22:37:51 cvs-tnt Exp $
*/
...
extern void send_command_packet_if();
extern void sort_new_mail();
extern int strpos2();
extern char strsub();

char tnt_bctempdir[MAXCHAR];
char tnt_bcsavedir[MAXCHAR];
char tnt_bcnewmaildir[MAXCHAR];
...
void validate(char *name)
{
  ...
  count = 1;
  while (!ok && (count < 999)) {
    sprintf(tmpname,"%s.%3.3d",name,count);
...
void crcfcs(u_char Data, unsigned short *crc)
{
  *crc = ((*crc) >> 8) ^ ccitt_table[((*crc) ^ Data) & 0xff];
}
   
void crcfbb_buf(u_char *adr, long size, unsigned short *crc)
{
  long ct;
...
void checksum16_buf(u_char *adr, long size, unsigned short *crc)
{
  long ct;
			...
			off_t start, off_t size)
{
  ...
  if (psize > fsize)
    psize = fsize;
  buf = (u_char *)malloc(psize);
  if (buf == NULL)
    return crc;
  k = sfopen(name, O_RDONLY);
  if (k != -1) {
    if (start > 0)
      lseek(k, start, 0);
    done = 0;
...
void filecut(char *filea, char *fileb, off_t start, off_t size)
{
  long err;
  u_char *puffer;
  off_t psize, done, bc, cstep, hli, fsize;
  short k1, k2;
  ...
  if (psize > fsize)
    psize = fsize;
  puffer = (u_char *)malloc(psize);
  if (puffer == NULL)
    return;
  done = 0;
  hli = 0;
  k1 = sfopen(filea, O_RDONLY);
  if (k1 != -1) {
    if (start > 0)
...
void ix2string(time_t zeit, char *timestring)
{
  ...
  cvtime = gmtime(&zeit);
  strftime(timestring,20,"%d.%m.%y %T",cvtime);
}
...
void ibcastprotokollcall(char *string)
{
  ...
  if (bcast_log_file[0] != '\0') {
    ...
    if (fp != NULL) {
      ...
      strftime(tmpstr,sizeof(tmpstr),"%d.%m.%y %T: ",cvtime);
      fprintf(fp,"%s%s\n",tmpstr,string);
      fclose(fp);
    }
  }
...
int analyse_bcast_body(char *buffer)
{
  ...
  if ((decbcast_flag) && (strcmp(srccall,ch_stat[0].mycall) != 0)) {
    tnc = 0;
    is_pacsat = dec_bcast((u_char)pid,tnc,srccall,destcall,vialist,
                          infosize,(u_char *)(buffer+1));
  }
  if (!is_pacsat) return(0);
  if (!shpacsat_flag) return(1); /* no display */
  infstart = decode_frameheader((u_char *)(buffer+1),infosize,bcastdec);
